...
def _extrair_por_regioes(
    ...
    """
    Recorta a imagem em regiões (position = minha área + vizinhos, community_cards, hole_cards),
    chama o LLM para cada uma com o prompt/schema da região e mescla os JSONs.
    """
    ...
    try:
        ...
        # 1) Região position (centro: minha área + vizinhos) → contexto (position, pot, total_number_of_players, risk)
...
def extrair_json_da_imagem(
    ...
    *,
    ...
    if use_regions:
        out = _extrair_por_regioes(caminho_imagem, llm_kwargs)
    else:
